...
func withInformer(ctx context.Context) (context.Context, controller.Informer) {
	f := factory.Get(ctx)
	inf := f.Servicecatalog().V1beta1().ClusterServiceClasses()
	return context.WithValue(ctx, Key{}, inf), inf.Informer()
}

// Get extracts the typed informer from the context.
func Get(ctx context.Context) v1beta1.ClusterServiceClassInformer {
	untyped := ctx.Value(Key{})
	if untyped == nil {
		logging.FromContext(ctx).Fatalf(
			"Unable to fetch %T from context.", (v1beta1.ClusterServiceClassInformer)(nil))
	}
	return untyped.(v1beta1.ClusterServiceClassInformer)
}
