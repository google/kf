...
const (
	...
	serviceAccountNameOverrideAnnotation = "apps.kf.dev/service-account-name"
)
...
func (r *Reconciler) ApplyChanges(ctx context.Context, app *v1alpha1.App) error {
	...
	// reconcile service account
	{
		...
		cfg, err := kfconfig.FromContext(ctx).Defaults()
		if err != nil {
			return fmt.Errorf("couldn't read config-defaults from context: %w", err)
		}

		overrideKsaName := ""
		if annotations := app.ObjectMeta.Annotations; len(annotations) > 0 {
			overrideKsaName = annotations[serviceAccountNameOverrideAnnotation]
		}

		switch {
		case cfg.AppEnableServiceAccountOverride && overrideKsaName != "":
			// User wants to override KSA.
			actual, err := r.serviceAccountLister.ServiceAccounts(app.Namespace).Get(overrideKsaName)
			if err != nil {
				return condition.MarkReconciliationError("getting override KSA", err)
			}

			app.Status.PropagateServiceAccountStatus(actual)

		default:
			// User wants Kf to manage the KSA.
			filteredSecretNames, err := r.getImagePullSecrets(space, app)
			if err != nil {
				return condition.MarkReconciliationError("getting pull secrets", err)
			}
...
// Fetches the secrets referenced in the build service account, and filters out the token secret auto-generated by K8s.
...
func (r *Reconciler) getImagePullSecrets(space *v1alpha1.Space, app *v1alpha1.App) ([]v1.LocalObjectReference, error) {
	buildSA, err := r.serviceAccountLister.ServiceAccounts(app.Namespace).Get(spaces.BuildServiceAccountName(space))
	if err != nil {
		return nil, fmt.Errorf("couldn't get build KSA: %w", err)
	}
	...
	for _, s := range buildSA.Secrets {
		...
		if err != nil {
			return nil, fmt.Errorf("couldn't get build KSA secret %s/%s: %w", app.Namespace, s.Name, err)
		}
