...
const (
	...
	nfsServiceFlag          = "enable_nfs_service"
	...
	adxBuildsFlag           = "enable_appdevexperience_builds"
)
...
func (fft FeatureFlagToggles) NfsService() *FeatureFlag {
	return &FeatureFlag{
		...
		Default: false,
		isEnabled: func(ff *FeatureFlag) bool {
			if setValue, ok := fft[nfsServiceFlag]; ok {
				return setValue
			}
			return ff.Default
		},
...
// AppDevExperienceBuilds returns the FeatureFlag for enabling the
// AppDevExperience Builds system (instead of the built-in Kf one).
...
// SetAppDevExperienceBuilds sets the value for the AppDevExperienceBuilds
// feature flag on a FeatureFlagToggles map.
