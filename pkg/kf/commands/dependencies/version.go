...
// newVersionCommand creates a command that gets the version for a dependency
func newVersionCommand(dependencies []dependency) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:        "Get the version for a dependency",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, dep := range dependencies {
				if dep.names().Has(depName) {
					version, err := dep.ResolveVersion()
					if err != nil {
						return err
					}
