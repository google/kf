...
func TestScheduleJob(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			assert: func(ctx context.Context, t *testing.T, buffer *bytes.Buffer, err error) {
				testutil.AssertContainsAll(
					t,
					buffer.String(),
					[]string{"WARNING: Scheduling an already scheduled Job."})
				client := fakeclient.Get(ctx)
				ts, err := client.KfV1alpha1().
					TaskSchedules(spaceName).
					Get(ctx, jobName, metav1.GetOptions{})
				testutil.AssertNil(t, "err", err)
				testutil.AssertEqual(t, "schedule", schedule, ts.Spec.Schedule)
				testutil.AssertEqual(t, "suspend", false, ts.Spec.Suspend)
			},
		},
		{
			name:  "TaskSchedule is suspended",
			space: spaceName,
			args:  []string{jobName, schedule},
			setup: func(ctx context.Context, t *testing.T) {
				client := fakeclient.Get(ctx)
			...
			assert: func(ctx context.Context, t *testing.T, buffer *bytes.Buffer, err error) {
				...
				testutil.AssertEqual(t, "schedule", schedule, ts.Spec.Schedule)
				testutil.AssertEqual(t, "suspend", false, ts.Spec.Suspend)
				testutil.AssertContainsAll(t, buffer.String(), []string{fmt.Sprintf("Job %s scheduled", jobName)})
			},
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			cmd := taskschedules.NewScheduleJobCommand(&config.KfParams{
				Space: tc.space,
			})
