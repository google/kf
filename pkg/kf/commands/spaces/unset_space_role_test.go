...
func TestNewUnsetSpaceRoleCommand(t *testing.T) {
	t.Parallel()
	const (
	...
	cases := map[string]struct {
		...
		"user not found in role": {
			space:   spaceName,
			args:    []string{"user1", "SpaceManager"},
			wantErr: errors.New("\"user1\" (User) was not found at Role \"SpaceManager\""),
		},
		"group not found in role": {
			space:   spaceName,
			args:    []string{"group1", "SpaceManager", "-t", "Group"},
			wantErr: errors.New("\"group1\" (Group) was not found at Role \"SpaceManager\""),
		},
		"user is removed from role": {
			space:           spaceName,
			args:            []string{"existed_user", "SpaceManager"},
			expectedStrings: []string{"\"existed_user\" (User) is removed from \"SpaceManager\" Role"},
		},
	}
	...
	for tn, tc := range cases {
		t.Run(tn, func(t *testing.T) {
			...
			c := NewUnsetSpaceRoleCommand(&config.KfParams{Space: tc.space}, k8sclient)
			c.SetOutput(buffer)
			c.SetArgs(tc.args)
			c.SetContext(ctx)
