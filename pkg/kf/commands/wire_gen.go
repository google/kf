...
func InjectPush(p *config.KfParams) *cobra.Command {
	...
	versionedInterface := config.GetTektonClient(p)
	kubernetesInterface := config.GetKubernetes(p)
	buildTailer := sources.TektonLoggingShim(versionedInterface, kubernetesInterface)
	...
	interface2 := config.GetServiceCatalogClient(p)
	clientInterface := servicebindings.NewClient(interface2)
...
func InjectListServices(p *config.KfParams) *cobra.Command {
	...
	appsGetter := provideAppsGetter(kfV1alpha1Interface)
	sourcesGetter := provideKfSources(kfV1alpha1Interface)
	interface2 := config.GetTektonClient(p)
	kubernetesInterface := config.GetKubernetes(p)
	buildTailer := sources.TektonLoggingShim(interface2, kubernetesInterface)
...
var SourcesSet = wire.NewSet(config.GetKfClient, sources.TektonLoggingShim, provideKfSources, sources.NewClient, config.GetTektonClient, config.GetKubernetes)
