...
// Rewrites a kf manifest to a Dockerfile.
...
func BuildToDocker(cfg *config.KfParams) *cobra.Command {
	cmd := &cobra.Command{
		Hidden:  true,
		Use:     "build-to-docker",
		Short:   "Create a dockerfile encapsulating the build.",
		Example: `kf build-to-docker`,
		Args:  cobra.ExactArgs(0),
		Long: `
		Produces a Dockerfile per-app.
		`,
