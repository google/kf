...
type xargsConfig struct {
	...
	// Example is the example to use for the command.
	Example string
	// ExecCommandDefault is the default command to exec on each resource
	ExecCommandDefault string
	// LabelFilters is flag name to label pairs to use as list filters.
	LabelFilters map[string]string
	// LabelRequirements is label requirements to filter resources.
	LabelRequirements []labels.Requirement
...
// ArgumentFilters returns the last set value for ArgumentFilters or the empty value
// if not set.
func (opts XargsOptions) ArgumentFilters() []ListArgumentFilter {
	return opts.toConfig().ArgumentFilters
}
...
// ExecCommandDefault returns the last set value for ExecCommandDefault or the empty value
// if not set.
func (opts XargsOptions) ExecCommandDefault() string {
	return opts.toConfig().ExecCommandDefault
}
...
// WithXargsArgumentFilters creates an Option that sets callbacks that can modify the lister.
func WithXargsArgumentFilters(val []ListArgumentFilter) XargsOption {
	return func(cfg *xargsConfig) {
		cfg.ArgumentFilters = val
	}
}
...
// WithXargsExecCommandDefault creates an Option that sets the default command to exec on each resource
func WithXargsExecCommandDefault(val string) XargsOption {
	return func(cfg *xargsConfig) {
		cfg.ExecCommandDefault = val
	}
}
