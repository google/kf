...
import (
	...
	cv1 "k8s.io/client-go/kubernetes/typed/networking/v1"
)
...
type Client interface {
	Create(ctx context.Context, namespace string, obj *v1.NetworkPolicy) (*v1.NetworkPolicy, error)
	Transform(ctx context.Context, namespace string, name string, transformer Mutator) (*v1.NetworkPolicy, error)
	Get(ctx context.Context, namespace string, name string) (*v1.NetworkPolicy, error)
	Delete(ctx context.Context, namespace string, name string) error
	List(ctx context.Context, namespace string) ([]v1.NetworkPolicy, error)
	Upsert(ctx context.Context, namespace string, newObj *v1.NetworkPolicy, merge Merger) (*v1.NetworkPolicy, error)
	WaitFor(ctx context.Context, namespace string, name string, interval time.Duration, condition Predicate) (*v1.NetworkPolicy, error)

	// Utility functions
	WaitForDeletion(ctx context.Context, namespace string, name string, interval time.Duration) (*v1.NetworkPolicy, error)
...
type coreClient struct {
	kclient cv1.NetworkPoliciesGetter
}

// Create inserts the given v1.NetworkPolicy into the cluster.
// The value to be inserted will be preprocessed and validated before being sent.
func (core *coreClient) Create(ctx context.Context, namespace string, obj *v1.NetworkPolicy) (*v1.NetworkPolicy, error) {
	return core.kclient.NetworkPolicies(namespace).Create(ctx, obj, metav1.CreateOptions{})
}
...
// conflict.
func (core *coreClient) Transform(ctx context.Context, namespace string, name string, mutator Mutator) (*v1.NetworkPolicy, error) {
	for {
		...
		result, err := core.kclient.NetworkPolicies(namespace).Update(ctx, obj, metav1.UpdateOptions{})
...
func (core *coreClient) Get(ctx context.Context, namespace string, name string) (*v1.NetworkPolicy, error) {
	res, err := core.kclient.NetworkPolicies(namespace).Get(ctx, name, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
...
func (core *coreClient) Delete(ctx context.Context, namespace string, name string) error {
	...
	if err := core.kclient.NetworkPolicies(namespace).Delete(ctx, name, metav1.DeleteOptions{PropagationPolicy: &foreground}); err != nil {
		return fmt.Errorf("couldn't delete the NetworkPolicy with the name %q: %v", name, err)
	}
...
func (core *coreClient) List(ctx context.Context, namespace string) ([]v1.NetworkPolicy, error) {
	res, err := core.kclient.NetworkPolicies(namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
		return nil, fmt.Errorf("couldn't list NetworkPolicys: %v", err)
	}
...
func (core *coreClient) Upsert(ctx context.Context, namespace string, newObj *v1.NetworkPolicy, merge Merger) (*v1.NetworkPolicy, error) {
	for ctx.Err() == nil {
		// kclient must be used so the error code can be validated by apierrors
		oldObj, err := core.kclient.NetworkPolicies(namespace).Get(ctx, newObj.Name, metav1.GetOptions{})
		...
		updated, err := core.kclient.NetworkPolicies(namespace).Update(ctx, merge(newObj, oldObj), metav1.UpdateOptions{})
		switch {
...
func (core *coreClient) WaitFor(ctx context.Context, namespace string, name string, interval time.Duration, condition Predicate) (*v1.NetworkPolicy, error) {
...
//
// The function polls infinitely if no timeout is supplied.
func (core *coreClient) waitForE(ctx context.Context, namespace string, name string, interval time.Duration, condition ConditionFuncE) (instance *v1.NetworkPolicy, err error) {
	var done bool
	...
	for {
		...
		case <-ctx.Done():
			return nil, errors.New("waiting for NetworkPolicy timed out")
		}
	}
}
...
func (core *coreClient) WaitForDeletion(ctx context.Context, namespace string, name string, interval time.Duration) (instance *v1.NetworkPolicy, err error) {
