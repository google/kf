...
// Predicate is a boolean function for a v1.ConfigMap.
type Predicate func(*v1.ConfigMap) bool

// Mutator is a function that changes v1.ConfigMap.
type Mutator func(*v1.ConfigMap) error

////////////////////////////////////////////////////////////////////////////////
// Client
////////////////////////////////////////////////////////////////////////////////

// Client is the interface for interacting with v1.ConfigMap types as ConfigMap CF style objects.
type Client interface {
	Create(ctx context.Context, namespace string, obj *v1.ConfigMap) (*v1.ConfigMap, error)
	Transform(ctx context.Context, namespace string, name string, transformer Mutator) (*v1.ConfigMap, error)
	Get(ctx context.Context, namespace string, name string) (*v1.ConfigMap, error)
	Delete(ctx context.Context, namespace string, name string) error
	List(ctx context.Context, namespace string) ([]v1.ConfigMap, error)
	Upsert(ctx context.Context, namespace string, newObj *v1.ConfigMap, merge Merger) (*v1.ConfigMap, error)
	WaitFor(ctx context.Context, namespace string, name string, interval time.Duration, condition Predicate) (*v1.ConfigMap, error)

	// Utility functions
	WaitForDeletion(ctx context.Context, namespace string, name string, interval time.Duration) (*v1.ConfigMap, error)
...
type coreClient struct {
	kclient cv1.ConfigMapsGetter
}

// Create inserts the given v1.ConfigMap into the cluster.
// The value to be inserted will be preprocessed and validated before being sent.
func (core *coreClient) Create(ctx context.Context, namespace string, obj *v1.ConfigMap) (*v1.ConfigMap, error) {
	return core.kclient.ConfigMaps(namespace).Create(ctx, obj, metav1.CreateOptions{})
}
...
// conflict.
func (core *coreClient) Transform(ctx context.Context, namespace string, name string, mutator Mutator) (*v1.ConfigMap, error) {
	for {
		...
		result, err := core.kclient.ConfigMaps(namespace).Update(ctx, obj, metav1.UpdateOptions{})
...
func (core *coreClient) Get(ctx context.Context, namespace string, name string) (*v1.ConfigMap, error) {
	res, err := core.kclient.ConfigMaps(namespace).Get(ctx, name, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
...
func (core *coreClient) Delete(ctx context.Context, namespace string, name string) error {
	...
	if err := core.kclient.ConfigMaps(namespace).Delete(ctx, name, metav1.DeleteOptions{PropagationPolicy: &foreground}); err != nil {
		return fmt.Errorf("couldn't delete the ConfigMap with the name %q: %v", name, err)
	}
...
func (core *coreClient) List(ctx context.Context, namespace string) ([]v1.ConfigMap, error) {
	res, err := core.kclient.ConfigMaps(namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
		return nil, fmt.Errorf("couldn't list ConfigMaps: %v", err)
	}
...
type Merger func(newObj, oldObj *v1.ConfigMap) *v1.ConfigMap
...
func (core *coreClient) Upsert(ctx context.Context, namespace string, newObj *v1.ConfigMap, merge Merger) (*v1.ConfigMap, error) {
	for ctx.Err() == nil {
		// kclient must be used so the error code can be validated by apierrors
		oldObj, err := core.kclient.ConfigMaps(namespace).Get(ctx, newObj.Name, metav1.GetOptions{})
		...
		updated, err := core.kclient.ConfigMaps(namespace).Update(ctx, merge(newObj, oldObj), metav1.UpdateOptions{})
		switch {
...
func (core *coreClient) WaitFor(ctx context.Context, namespace string, name string, interval time.Duration, condition Predicate) (*v1.ConfigMap, error) {
...
//
// The function polls infinitely if no timeout is supplied.
func (core *coreClient) waitForE(ctx context.Context, namespace string, name string, interval time.Duration, condition ConditionFuncE) (instance *v1.ConfigMap, err error) {
	var done bool
	...
	for {
		...
		case <-ctx.Done():
			return nil, errors.New("waiting for ConfigMap timed out")
		}
	}
}
...
func (core *coreClient) WaitForDeletion(ctx context.Context, namespace string, name string, interval time.Duration) (instance *v1.ConfigMap, err error) {
