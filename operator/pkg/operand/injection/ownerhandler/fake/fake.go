...
import (
	...
	client "kf-operator/pkg/operand/injection/ownerhandler"
...
func With(ctx context.Context) (context.Context, fake.OwnerHandler) {
	f := fake.CreateHandler()
	return context.WithValue(ctx, client.Key{}, f), f
}

// Get returns OwnerInjector from ctx.
func Get(ctx context.Context) fake.OwnerHandler {
	untyped := ctx.Value(client.Key{})
	if untyped == nil {
		logging.FromContext(ctx).Panic(
			"Unable to fetch fake.OwnerInjector from context.")
	}
	return untyped.(fake.OwnerHandler)
}
