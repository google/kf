...
func withClient(ctx context.Context, cfg *rest.Config) context.Context {
	return context.WithValue(ctx, Key{}, operand.CreateOwnerHandlerWithCtx(ctx))
}

// Get returns OwnerHandler from ctx.
func Get(ctx context.Context) operand.OwnerHandler {
	untyped := ctx.Value(Key{})
	if untyped == nil {
		logging.FromContext(ctx).Panic(
			"Unable to fetch mf.Client from context.")
	}
	return untyped.(operand.OwnerHandler)
}
