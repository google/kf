...
// controller reconciling v1alpha1.ClusterActiveOperand.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.ClusterActiveOperand. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.ClusterActiveOperand) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.ClusterActiveOperand.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.ClusterActiveOperand. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.ClusterActiveOperand) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.ClusterActiveOperand if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.ClusterActiveOperand.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.ClusterActiveOperand) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.ClusterActiveOperand) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha1.ClusterActiveOperand resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister operandv1alpha1.ClusterActiveOperandLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1alpha1.ClusterActiveOperand, desired *v1alpha1.ClusterActiveOperand) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.OperandV1alpha1().ClusterActiveOperands()
		...
		updater := r.Client.OperandV1alpha1().ClusterActiveOperands()
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.ClusterActiveOperand, desiredFinalizers sets.Set[string]) (*v1alpha1.ClusterActiveOperand, error) {
...
func (r *reconcilerImpl) updateFinalizersFilteredServerSideApply(ctx context.Context, resource *v1alpha1.ClusterActiveOperand, desiredFinalizers sets.Set[string]) (*v1alpha1.ClusterActiveOperand, error) {
	...
	patcher := r.Client.OperandV1alpha1().ClusterActiveOperands()
...
func (r *reconcilerImpl) updateFinalizersFilteredMergePatch(ctx context.Context, resource *v1alpha1.ClusterActiveOperand, desiredFinalizers sets.Set[string]) (*v1alpha1.ClusterActiveOperand, error) {
	// Don't modify the informers copy.
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.ClusterActiveOperand) (*v1alpha1.ClusterActiveOperand, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.ClusterActiveOperand, reconcileEvent reconciler.Event) (*v1alpha1.ClusterActiveOperand, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
