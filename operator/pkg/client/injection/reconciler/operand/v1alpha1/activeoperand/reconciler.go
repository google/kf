...
// controller reconciling v1alpha1.ActiveOperand.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.ActiveOperand. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.ActiveOperand) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.ActiveOperand.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.ActiveOperand. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.ActiveOperand) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.ActiveOperand if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.ActiveOperand.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.ActiveOperand) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.ActiveOperand) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha1.ActiveOperand resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister operandv1alpha1.ActiveOperandLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.ActiveOperands(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1alpha1.ActiveOperand, desired *v1alpha1.ActiveOperand) error {
	...
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		...
		if attempts > 0 {
			...
			getter := r.Client.OperandV1alpha1().ActiveOperands(desired.Namespace)
		...
		updater := r.Client.OperandV1alpha1().ActiveOperands(existing.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.ActiveOperand) (*v1alpha1.ActiveOperand, error) {
