...
func CreateConfigForTest(defaults *kf.DefaultsConfig) *Config {
	cfg := &Config{}
...
func DefaultConfigContext(ctx context.Context) context.Context {
	return toContext(ctx, &Config{
		defaults: kf.BuiltinDefaultsConfig(),
	})
}
	...
	store := &Store{
		UntypedStore: configmap.NewUntypedStore(
			...
			configmap.Constructors{
				kf.DefaultsConfigName: kf.NewDefaultsConfigFromConfigMap,
			},
			onAfterStore...,
		),
	}
...
func (s *Store) waitToLoad() *Config {
	if s.UntypedLoad(kf.DefaultsConfigName) == nil {
		return &Config{
			loadError: errors.New("error loading defaults config, value is nil"),
		}
	}
	return &Config{
		defaults: s.UntypedLoad(kf.DefaultsConfigName).(*kf.DefaultsConfig),
	}
}
