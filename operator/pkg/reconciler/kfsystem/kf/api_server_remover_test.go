...
func TestAPIServerRemoverTransformer(t *testing.T) {
	t.Parallel()
		...
		},
		{
			...
			want: mftesting.ToUnstructured(
				k8s.ConfigMap(
					...
					k8s.WithData("placeholder", "This config map implies that the operator didn't see the necessary pods for the API Service."),
					k8s.WithConfigMapNamespace("kf"),
				),
			),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			transformer := APIServerRemoverTransformer(
				context.Background(),
			)
